...
import { ALL_STABLE_POOL_IDS, NEARX_POOL_ID } from "@/services/swap/swapConfig";
import { getPoolsByIds, getYourPools } from "@/services/indexer";
import { UserLiquidityInfo, get_liquidity_value } from "@/services/commonV3";
...
function useUnClaimed() {
  ...
  async function getUnClaimedRewards() {
    ...
    const userUncliamedRewards: { [key: string]: any } = {};
    ...
    const unclaimed_token_meta_datas: { [key: string]: TokenMetadata } = {};
  ...
  async function get_all_liquidities_details() {
    const promise_list_details = liquidities_list.map(
      (item: UserLiquidityInfo) => {
        return get_liquidity(item.lpt_id as string);
      }
    );
