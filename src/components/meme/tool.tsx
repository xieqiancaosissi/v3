...
export function getSeedApr(seed: Seed) {
  ...
  farms?.forEach(function (item: FarmBoost) {
    ...
    if (allPendingFarms || (!allPendingFarms && !pendingFarm)) {
      apr = apr.plus(item.apr || 0);
    }
  });
  return apr.mul(100).toFixed();
}
export function getTotalRewardBalance(seed: Seed, extraBalance: any) {
...
export function formatSecondsAbb(seconds: number) {
  const days = Math.floor(seconds / (60 * 60 * 24));
  const hours = Math.floor((seconds % (60 * 60 * 24)) / (60 * 60));
  const minutes = Math.floor((seconds % (60 * 60)) / 60);
  let result = "";
...
export function getAccountFarmData({
  ...
  tokenPriceList: Record<string, TokenPrice>;
  xrefSeeds: Record<string, Seed>;
  ...
  let memeUnclaimed: Record<string, string> = {};
  let xrefUnclaimed: Record<string, string> = {};
  const xrefContractId =
    MEME_TOKEN_XREF_MAP[seed_id as keyof typeof MEME_TOKEN_XREF_MAP];
  if (!xrefContractId) {
    return;
  }
  ...
  ).reduce((acc: Record<string, string>, [tokenId, amount]) => {
    return {
      ...acc,
      ...{
...
export function getListedMemeSeeds(
  ...
  const memeListedSeeds: Record<string, Seed> = {};
  meme_winner_tokens.forEach((memeSeedId: string) => {
...
export function sortByXrefStaked(xrefSeeds: Record<string, Seed>) {
  const { MEME_TOKEN_XREF_MAP } = getMemeContractConfig();
  return (memeTokenIdB: string, memeTokenIdA: string) => {
    const xrefSeedB = xrefSeeds?.[MEME_TOKEN_XREF_MAP[memeTokenIdB] as string];
    const xrefSeedA = xrefSeeds?.[MEME_TOKEN_XREF_MAP[memeTokenIdA] as string];
    return (
...
export function formatLineUi(v: string) {
  if (!v || v == "0" || v == "$0" || v == "$-") return "-";
  return v;
}
export function isLineUi(v: string) {
  if (formatLineUi(v) == "-") return true;
  return false;
}
export function emptyObject(o: Record<string, any>) {
  if (o && Object.keys(o).length) return false;
  return true;
}
export function emptyNumber(n: any) {
