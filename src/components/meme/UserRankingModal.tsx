...
function UserRankingModal(props: any) {
  ...
  const [tableDate, setTableDate] = useState<any | null>(null);
  ...
  const getCustomPaginationRange = (current: number, total: number) => {
    if (is_mobile) {
      const pages = [];
      if (total <= 3) {
  ...
  let timeoutId: any;
  const handleMouseEnterRow = (itemIndex: any) => {
  ...
  //   setCurrentPage(1);
  // };
  const getPopupPosition = (triggerElement: any) => {
    if (triggerElement) {
  ...
  return (
    ...
    >
      <div
        className="py-5 px-4 text-base text-gray-50 bg-dark-10 rounded-2xl "
        style={{
          width: cardWidth,
          height: cardHeight,
        }}
      >
        ...
        <div className="text-sm text-left mb-1.5 px-6 flex items-center xsm:justify-between xsm:border-b xsm:border-memeBorderColor xsm:pb-3 xsm:px-0">
              ...
              <div className="flex justify-center items-center">
                <div
                  className={`flex justify-center items-center mr-6 xsm:mr-3 cursor-pointer lg:hidden ${
                    currentPage === 1 ? "text-gray-60" : "text-white"
                  }`}
                  onClick={() => {
              ...
              <div className="flex justify-center items-center">
                ...
                <div
                  className={`flex justify-center items-center ml-6 xsm:ml-3 cursor-pointer lg:hidden ${
                    currentPage === totalPages ? "text-gray-60" : "text-white"
                  }`}
                  onClick={() => {
                    if (currentPage < totalPages) {
                      paginate(totalPages);
        ...
        paginationRange.map((page, index) => {
          if (typeof page === "number") {
            ...
            return (
              <span
                key={index}
                className="mr-3.5 py-1 px-2 xsm:px-1.5 rounded-md text-gray-60 cursor-default"
              >
                {page}
              </span>
            );
